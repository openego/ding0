...
CREATE TABLE buildings_filtered as
    select pop.osm_id, pop.amenity, pop.building, pop."name", st_transform(ST_SetSRID(pop.way, 3857), 4326) as geometry, pop.way_area as "area", st_transform(ST_SetSRID(st_centroid(pop.way), 3857), 4326) as geo_center, pop.tags
...
CREATE TABLE buildings_with_res_tmp as
    select * from (
        select buildings.osm_id, buildings.amenity, buildings.building, buildings.name, buildings.geometry, buildings.area, buildings.geo_center, buildings.tags, zensus_apartments.n_apartments, zensus_apartments.idx as id_of_n_apartments
...
drop table if exists buildings_with_res

create table buildings_with_res as
    select buildings_with_res_tmp.osm_id, buildings_with_res_tmp.amenity, buildings_with_res_tmp.building, buildings_with_res_tmp.name, buildings_with_res_tmp.geometry, buildings_with_res_tmp.area, buildings_with_res_tmp.geo_center, buildings_with_res_tmp.tags, buildings_with_res_tmp.n_apartments, buildings_with_res_tmp.id_of_n_apartments, bwa.n_apartments_in_n_buildings
...
-- create index
CREATE INDEX ON buildings_with_res USING gist (geometry);
...
CREATE TABLE buildings_with_amenities as
    select bwa.osm_id, bwa.building, bwa.area, bwa.geometry, bwa."name", bwa.building_tags, bwa.amenity_tags, bwa.n_amenities_inside, case when n_apartments>0 then bwa.n_apartments / bwa.n_apartments_in_n_buildings else 0 end as n_apartments
    from (
        select bwa.osm_id, bwa.building, bwa.area, bwa.geometry, bwa."name", bwa.building_tags, bwa.amenity_tags, bwa.n_amenities_inside, SUM(bwa.n_apartments) as n_apartments, SUM(bwa.n_apartments_in_n_buildings) as n_apartments_in_n_buildings
        from (
            select b.osm_id_amenity as osm_id, coalesce(b.amenity, b.building) as building, b.area, b.geometry_amenity as geometry, b.name, b.building_tags, b.amenity_tags, coalesce(b.n_apartments, 0) as n_apartments, coalesce(b.n_apartments_in_n_buildings, 0) as n_apartments_in_n_buildings, ainb.n_amenities_inside
        ...
        group by bwa.osm_id, bwa.building, bwa.area, bwa.geometry, bwa."name", bwa.building_tags, bwa.amenity_tags, bwa.n_amenities_inside
    ) bwa


CREATE INDEX ON buildings_with_amenities USING gist (geometry);
...
CREATE TABLE buildings_without_amenities as
    select bwa.osm_id, bwa.building, bwa.area, bwa.geometry, bwa."name", bwa.tags, case when n_apartments>0 then bwa.n_apartments / bwa.n_apartments_in_n_buildings else 0 end as n_apartments
    from (
        ...
        from (
            select bf.osm_id, coalesce(bf.amenity, bf.building) as building, bf.name, bf.area, bf.geo_center as geometry, bf.tags, coalesce(bf.n_apartments, 0) as n_apartments, coalesce(bf.n_apartments_in_n_buildings, 0) as n_apartments_in_n_buildings
            from buildings_with_res bf
...
CREATE TABLE amenities_not_in_buildings as
    ...
    where af.osm_id not in (select aib.osm_id_amenity from amenities_in_buildings aib)
