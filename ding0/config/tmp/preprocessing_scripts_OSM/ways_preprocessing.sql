...
CREATE TABLE ways_with_segments as
    ...
    from (
        with way as (
            SELECT wp.osm_id, wp.nodes, wp.geometry, ST_DumpPoints(wp.geometry) as geo_dump FROM ways_preprocessed wp)
        SELECT way_w_segments.osm_id, way_w_segments.nodes, way_w_segments.geometry, array_agg(ST_Length(way_w_segments.linestring_segment::geography)) as length_segments
        FROM (
            SELECT way.osm_id, way.nodes, way.geometry, ST_AsText(ST_MakeLine(lag((geo_dump).geom, 1, NULL) OVER (PARTITION BY way.osm_id ORDER BY way.osm_id, (geo_dump).path), (geo_dump).geom)) AS linestring_segment from way) way_w_segments
        WHERE way_w_segments.linestring_segment IS NOT null
        GROUP BY way_w_segments.osm_id, way_w_segments.nodes, way_w_segments.geometry
    ) ways
