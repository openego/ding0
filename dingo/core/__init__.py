...
class NetworkDingo:
    ...
    def run_dingo(self, conn, mv_grid_districts_no=None, debug=False):
        ...
        if debug:
            logger.info(
                'Elapsed time for {0} MV Grid Districts (seconds): {1}'.format(
        ...
        for mv_grid_district in self.mv_grid_districts():
            ...
            for lv_load_area in mv_grid_district.lv_load_areas():
                ...
                for lv_grid_district in lv_load_area.lv_grid_districts():
                    lv_grid_districts_dict[
                        lv_grid_district.id_db] = lv_grid_district
                    lv_stations_dict[
                        lv_grid_district.lv_grid.station().id_db] = lv_grid_district.lv_grid.station()
    ...
    def import_mv_grid_districts(self, conn, mv_grid_districts_no=None):
        ...
        session = Session()
        grid_districts = session.query(
            self.orm['orm_mv_grid_districts'].subst_id,
            ...
            label('subs_geom')). \
            join(self.orm['orm_mv_stations'],
                 self.orm['orm_mv_grid_districts'].subst_id ==
                 self.orm['orm_mv_stations'].subst_id). \
            filter(self.orm['orm_mv_grid_districts'].subst_id.in_(
    ...
    def import_lv_load_areas(self, conn, mv_grid_district, lv_grid_districts,
        ...
        lv_load_areas_sqla = session.query(
            ...
            func.ST_AsText(
                func.ST_Transform(self.orm['orm_lv_load_areas'].geom, srid)). \
                label('geo_area'),
            func.ST_AsText(
                func.ST_Transform(self.orm['orm_lv_load_areas'].geom_centre,
                                  srid)). \
                label('geo_centre'),
    ...
    def import_lv_stations(self, conn):
        ...
        lv_stations_sqla = session.query(
            self.orm['orm_lv_stations'].mvlv_subst_id,
    ...
    def import_generators(self, conn, debug=False):
        ...
        def import_res_generators():
            ...
            generators_sqla = session.query(
                ...
                func.ST_AsText(func.ST_Transform(
                    self.orm['orm_re_generators'].rea_geom_new, srid)).label(
                    'geom_new'),
                func.ST_AsText(func.ST_Transform(
                    self.orm['orm_re_generators'].geom, srid)).label('geom')
            ). \
                filter(
                self.orm['orm_re_generators'].subst_id.in_(
                    list(mv_grid_districts_dict))). \
                filter(
                self.orm['orm_re_generators'].voltage_level.in_([4, 5, 6, 7])). \
                ...
                # LV generators
                    ...
                    if mvlv_subst_id and not isnan(mvlv_subst_id):
                        ...
                        # this occurs due to exclusion of LA with peak load < 1kW
                            ...
                            lv_grid_district = random.choice(
                                list(lv_grid_districts_dict.values()))
        ...
        def import_conv_generators():
            ...
            generators_sqla = session.query(
                ...
                filter(
                self.orm['orm_conv_generators'].subst_id.in_(
                    list(mv_grid_districts_dict))). \
                filter(
                self.orm['orm_conv_generators'].voltage_level.in_([4, 5, 6])). \
    ...
    def import_static_data(self):
        ...
        static_data['MV_trafos'] = pd.read_csv(
        ...
        static_data['MV_overhead_lines'] = pd.read_csv(
            os.path.join(package_path, 'data',
                         equipment_mv_parameters_lines),
            comment='#',
        ...
        static_data['MV_cables'] = pd.read_csv(
            os.path.join(package_path, 'data',
                         equipment_mv_parameters_cables),
            comment='#',
            converters={'I_max_th': lambda x: int(x),
        ...
        static_data['LV_cables'] = pd.read_csv(
            os.path.join(package_path, 'data',
                         equipment_lv_parameters_cables),
            comment='#',
            index_col='name',
        ...
        static_data['LV_trafos'] = pd.read_csv(
            os.path.join(package_path, 'data',
                         equipment_lv_parameters_trafos),
            comment='#',
            delimiter=',',
        ...
        static_data['LV_model_grids_strings'] = pd.read_csv(
            os.path.join(package_path, 'data',
                         model_grids_lv_string_properties),
            comment='#',
            delimiter=';',
        ...
        static_data['LV_model_grids_strings_per_grid'] = pd.read_csv(
            os.path.join(package_path, 'data',
                         model_grids_lv_apartment_string),
            comment='#',
            delimiter=';',
    ...
    def import_orm(self):
        ...
        if data_source == 'model_draft':
            orm['orm_mv_grid_districts'] = orm_model_draft.__getattribute__(
                mv_grid_districts_name)
            orm['orm_mv_stations'] = orm_model_draft.__getattribute__(
                mv_stations_name)
            orm['orm_lv_load_areas'] = orm_model_draft.__getattribute__(
                lv_load_areas_name)
            orm['orm_lv_grid_district'] = orm_model_draft.__getattribute__(
                lv_grid_district_name)
            orm['orm_lv_stations'] = orm_model_draft.__getattribute__(
                lv_stations_name)
            orm['orm_conv_generators'] = orm_model_draft.__getattribute__(
                conv_generators_name)
            orm['orm_re_generators'] = orm_model_draft.__getattribute__(
                re_generators_name)
            ...
            orm['orm_mv_grid_districts'] = orm_grid.__getattribute__(
                mv_grid_districts_name)
            orm['orm_mv_stations'] = orm_grid.__getattribute__(mv_stations_name)
            orm['orm_lv_load_areas'] = orm_demand.__getattribute__(
                lv_load_areas_name)
            orm['orm_lv_grid_district'] = orm_grid.__getattribute__(
                lv_grid_district_name)
            orm['orm_lv_stations'] = orm_grid.__getattribute__(lv_stations_name)
            orm['orm_conv_generators'] = orm_supply.__getattribute__(
                conv_generators_name)
            orm['orm_re_generators'] = orm_supply.__getattribute__(
                re_generators_name)
            ...
            logger.error(
                "Invalid data source {} provided. Please re-check the file "
        ...
        for grid_district in self.mv_grid_districts():
            ...
            for node in grid_district.mv_grid._graph.nodes():
                if isinstance(node, LVLoadAreaCentreDingo):
                    lv_load_area_centres.append(
                        (node.geo_data.x, node.geo_data.y))
                elif isinstance(node, MVCableDistributorDingo):
                    mv_cable_distributors.append(
                        (node.geo_data.x, node.geo_data.y))
            ...
            if lv_load_area_centres:
                lv_load_area_centres_wkb = from_shape(
                    MultiPoint(lv_load_area_centres), srid=srid)
            else:
            ...
            if mv_circuit_breakers:
                mv_circuit_breakers_wkb = from_shape(
                    MultiPoint(mv_circuit_breakers), srid=srid)
            else:
            ...
            if mv_generators:
                mv_generators_wkb = from_shape(MultiPoint(mv_generators),
                                               srid=srid)
            else:
            ...
            for lv_load_area in grid_district.lv_load_areas():
                for lv_grid_district in lv_load_area.lv_grid_districts():
                    ...
                    if station not in grid_district.mv_grid.graph_isolated_nodes():
                        lv_stations.append(
                            (station.geo_data.x, station.geo_data.y))
        ...
        for mv_grid_district in self.mv_grid_districts():
            for load_area in mv_grid_district.lv_load_areas():
                if not load_area.is_aggregated:
                    ...
                    logger.info(
                        '{} is of type aggregated. No grid is created.'.format(
                            repr(load_area)))
    ...
    def run_powerflow(self, conn, method='onthefly', export_pypsa=False,
                      debug=False):
